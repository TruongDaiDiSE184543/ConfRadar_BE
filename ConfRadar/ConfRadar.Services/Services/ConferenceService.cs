...
namespace ConfRadar.Services.Services
{
    ...
    public class ConferenceService : IConferenceService
    {
        ...
        public async Task<bool> ChangeConferenceStatus(string userId, string conferenceId, string newStatus, string? reason = null)
        {
            ...
            if (conference == null) throw new BadRequestException($"Không tìm th?y h?i ngh? v?i id {conferenceId}");
            if (conference.CreatedBy != userId) throw new BadRequestException("Ch? có ngu?i t?o ra conference m?i thay d?i du?c tr?ng thái");
            ...
            if (newStatusEntity == null) throw new BadRequestException($"Không tim th?y conference status v?i ID {newStatus}");
            ...
            if (conference.ConferenceStatusId == pendingStatus.ConferenceStatusId && newStatusEntity.ConferenceStatusId != deleteStatus.ConferenceStatusId) throw new Exception("Conference c?n Organizer approve lên preparing first d? có th? thay d?i tr?ng thái");
            if (conference.ConferenceStatusId == draftStatus.ConferenceStatusId && newStatusEntity.ConferenceStatusId != deleteStatus.ConferenceStatusId) throw new Exception("Hi?n t?i b?n draft c?a conference ch? có th? chuy?n sang delete.Conference c?n request lên pending d? Organizer approve lên preparing first d? có th? thay d?i tr?ng thái khác");
        ...
        public async Task<bool> UpdateConferenceStatusAsync(string conferenceId, string newStatusName, string? reason = null)
        {
            ...
            if (conference == null)
            {
                throw new BadRequestException("Không tìm th?y conf id này");
            }
            ...
            {
                throw new BadRequestException("Không tìm th?y tr?ng thái hi?n t?i c?a h?i ngh?");
            }
            ...
            if (newStatus == null)
            {
                throw new BadRequestException($"Không t?n t?i tr?ng thái {newStatus}");
            }
            ...
            try
            {
                ...
                {
                    throw new BadRequestException($"Chuy?n tr?ng thái t? '{currentStatus.ConferenceStatusName}' sang '{newStatusName}' không h?p l?");
                }
            ...
            string? userId = null, bool isOrganizer = false)
        {
            ...
            {
                throw new BadRequestException("Organizers không du?c phép xem ho?c l?c theo tr?ng thái 'Draft'.");
            }
            if (isOrganizer)
            {
        ...
        public async Task<int> SubmitConferenceFeedback(CreateConferenceFeedbackRequest request, string userId)
        {
            ...
            {
                throw new BadRequestException($"Không tìm th?y phiên v?i mã {request.ConferenceSessionId}");
            }
            ...
            {
                throw new BadRequestException($"B?n chua mua b?t c? vé nào nên không th? dánh giá");
            }
            ...
            {
                throw new NotFoundException($"Không tìm th?y tr?ng thái check in trong h? th?ng");
            }
            if (userCheckInFound.CheckinStatusId == pendingCheckInStatus.CheckinStatusId)
            {
                throw new BadRequestException($"B?n ph?i check in r?i m?i du?c dánh giá");
            }
        ...
        public async Task<List<ConferenceDetailForScheduleResponse>> GetListConferencesForScheduleByUserId(string userId)
        {
            ...
            {
                throw new NotFoundException("Không tìm th?y tr?ng thái ready cho h?i ngh?");
            }
        ...
        public async Task<bool> RequestOrganizerApproval(string confId, string userId)
        {
            ...
            if (conference == null) throw new BadRequestException($"Không tìm th?y h?i ngh? v?i ID: {confId}");
            if (conference.CreatedBy != userId) throw new BadRequestException("B?n không có quy?n g?i yêu c?u du?c approve cho h?i ngh? này");
            var draftStatus = await _unitOfWork.ConferenceStatusRepository.GetConferenceStatusByNameAsync(ConferenceStatusEnum.Draft.GetDescription());
            if (conference.ConferenceStatusId != draftStatus.ConferenceStatusId) throw new BadRequestException($" conference v?i ID {confId} ph?i dang là draft status m?i có th? yêu c?u duy?t du?c");
            var pendingStatus = await _unitOfWork.ConferenceStatusRepository.GetConferenceStatusByNameAsync(ConferenceStatusEnum.Pending.GetDescription());
        ...
        {
            #region === 1. L?Y D? LI?U VÀ VALIDATION CO B?N ===

            var conference = await _unitOfWork.ConferenceRepository.GetConferenceByIdAsync(confId);
            if (conference == null)
                throw new NotFoundException($"Không tìm th?y h?i ngh? v?i ID {confId}");

            // 1.1. Phân quy?n và lo?i h?i ngh?
            if (conference.CreatedBy != userId)
                throw new BadRequestException("B?n không có quy?n kích ho?t ch? d? waitlist cho h?i ngh? này.");
            if (conference.IsResearchConference != true)
                throw new BadRequestException("Ch?c nang này ch? dành cho h?i ngh? nghiên c?u.");

            // 1.2. L?y các Phase và ki?m tra s? t?n t?i
            ...
            if (notWaitlistPhase == null || waitlistPhase == null)
                throw new InvalidOperationException("H?i ngh? chua du?c c?u hình d?y d? phase chính và phase waitlist.");

            // 1.3. Ki?m tra xem waitlist dã du?c kích ho?t chua
            if (waitlistPhase.IsActive == true) // Ch? c?n ki?m tra phase waitlist là d?
                throw new BadRequestException($"Waitlist cho h?i ngh? này dã du?c kích ho?t tru?c dó.");

            // 1.4. L?y Research Detail (c?n cho các bu?c sau)
            ...
            if (researchDetail == null)
                throw new InvalidOperationException($"H?i ngh? chua có chi ti?t nghiên c?u (Research Detail).");
