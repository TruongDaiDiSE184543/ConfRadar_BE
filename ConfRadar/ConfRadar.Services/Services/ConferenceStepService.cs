...
namespace ConfRadar.Services.Services
{
        ...
        private static readonly HashSet<string> AllowedPaperFormats = new HashSet<string>
    {
        ...
        // DÁN PHUONG TH?C NÀY VÀO SERVICE C?A B?N (CÓ TH? Ð?T TRONG VÙNG HELPER)
        ...
        private async Task<ConferenceSession> UpdateSessionInternalAsync(string sessionId, UpdateConferenceSessionRequest request, string userId)
        {
            ...
            if (session == null) throw new NotFoundException($"Không tìm th?y phiên v?i ID {sessionId}");

            var conference = await _unitOfWork.ConferenceRepository.GetConferenceByIdAsync(session.ConferenceId);

            #region === 1. VALIDATION ===
            // 1.1. Phân quy?n và tr?ng thái
            if (conference.CreatedBy != userId)
                throw new ForbiddenException("B?n không có quy?n c?p nh?t phiên này.");
            await EnsureConferenceIsEditable(conference);

            // 1.2. VALIDATION NGHI?P V? QUAN TR?NG: Ki?m tra d? li?u ph? thu?c
            ...
            {
                // N?u dã có bài báo du?c gán, c?m thay d?i các thông tin quan tr?ng v? l?ch trình
                if (request.Date.HasValue || request.StartTime.HasValue || request.EndTime.HasValue || request.RoomId != null)
                {
                    throw new BadRequestException("Không th? thay d?i th?i gian ho?c d?a di?m c?a phiên này vì dã có bài báo du?c gán d? trình bày.");
                }
            }

            // 1.3. Xác d?nh các giá tr? cu?i cùng và validation ngày tháng
            ...
            if (finalDate < conference.StartDate || finalDate > conference.EndDate)
                throw new BadRequestException($"Ngày c?a phiên ({finalDate:dd/MM/yyyy}) ph?i n?m trong kho?ng th?i gian di?n ra h?i ngh?.");

            var finalStartDateTime = finalDate.ToDateTime(finalStartTime);
            var finalEndDateTime = finalDate.ToDateTime(finalEndTime);

            // 1.4. Ki?m tra trùng l?p th?i gian v?i các session khác trong DB
            ...
            // Ch? c?p nh?t các tru?ng này n?u an toàn (chua có bài báo gán vào)
            if (!assignedPapers.Any())
            {
                ...
                foreach (CreateConferencePriceRequest toBeConferencePrice in conferencePriceRequest)
                {
                    ...
                    foreach (CreatePricePhaseRequest createPricePhaseRequest in toBeConferencePrice.Phases)
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(createPricePhaseRequest.PhaseName))
                            throw new BadRequestException($"Tên giai do?n trong vé '{createPricePhaseRequest.PhaseName}' không du?c d? tr?ng.");
                        ...
                        if (createPricePhaseRequest.refundInPhase != null && createPricePhaseRequest.refundInPhase.Any())
                        {
                            ...
                            for (int i = 0; i < sortedRefunds.Count; i++)
                            {
                                ...
                                if (refundRequest.PercentRefund.Value < 0 || refundRequest.PercentRefund.Value > 100)
                                    throw new BadRequestException("PerentRefund ph?i n?m trong kho?ng 0-100");
                                ...
                                {
                                    throw new BadRequestException($"Trong giai do?n '{createPricePhaseRequest.PhaseName}', h?n chót hoàn ti?n ({refundRequest.RefundDeadline.Value:dd/MM/yyyy}) ph?i sau ngày b?t d?u giai do?n ({createPricePhaseRequest.StartDate:dd/MM/yyyy}).");
                                }
                                // 2. Ph?i tru?c ngày b?t d?u bán vé c?a c? h?i ngh?
                                if (refundRequest.RefundDeadline.Value >= conference.TicketSaleEnd)
                                {
                                    throw new BadRequestException($"Trong giai do?n '{createPricePhaseRequest.PhaseName}', h?n chót hoàn ti?n ({refundRequest.RefundDeadline.Value:dd/MM/yyyy}) ph?i tru?c ngày k?t thúc bán vé c?a h?i ngh? ({conference.TicketSaleEnd:dd/MM/yyyy}).");
                                }
        ...
        public async Task<List<ConferenceSessionWithMediaResponse>> AddConferenceSessionsAsync(string conferenceId, AddConferenceSessionsRequest request, string userId)
        {
            ...
            if (conference == null)
            {
                throw new BadRequestException($"Không tìm th?y h?i ngh? v?i ID {conferenceId}");
            }
            ...
            if (conference.CreatedBy != userId)
            {
                throw new BadRequestException("B?n không có quy?n thêm phiên (session) cho h?i ngh? này.");
            }
            if (request.Sessions == null || !request.Sessions.Any())
            {
                throw new BadRequestException("Yêu c?u ph?i ch?a ít nh?t m?t phiên (session).");
            }
                ...
                if (request.Sessions != null)
                {
                    foreach (var session in request.Sessions)
                    {
                        if (string.IsNullOrWhiteSpace(session.Title))
                            throw new BadRequestException("Tiêu d? c?a phiên không du?c d? tr?ng.");
                        if (session.RoomId == null || session.StartTime == null || session.EndTime == null || session.Date == null)
                            throw new BadRequestException($"Phiên '{session.Title}' c?n có d? RoomId, StartTime, EndTime, và Date.");

                        if (await _unitOfWork.RoomRepository.GetRoomByIdAsync(session.RoomId) == null)
                            throw new NotFoundException($"Phòng v?i ID {session.RoomId} không t?n t?i.");
                        ...
                        if (session.Speaker != null)
                        {
                            foreach (var speakerRequest in session.Speaker)
                            {
                                if (string.IsNullOrWhiteSpace(speakerRequest.Name))
                                    throw new BadRequestException($"Tên c?a di?n gi? trong phiên '{session.Title}' không du?c d? tr?ng.");
                                if (speakerRequest.Image != null && !_objectStorageFileService.IsValidImageFile(speakerRequest.Image))
                                    throw new BadRequestException($"Ð?nh d?ng ?nh c?a di?n gi? '{speakerRequest.Name}' không du?c h? tr?.");
        ...
        //    if (session == null)
        //    {
        //        throw new NotFoundException($"Không tìm th?y phiên v?i ID {sessionId}");
        //    }
        ...
        //    {
        //        throw new BadRequestException($"Ngày c?a phiên ({newDate:dd/MM/yyyy}) ph?i n?m trong kho?ng th?i gian di?n ra h?i ngh? ({conference.StartDate:dd/MM/yyyy} - {conference.EndDate:dd/MM/yyyy}).");
        //    }
        ...
        public async Task<ConferenceSessionWithMediaResponse> UpdateConferenceSessionAsync(string sessionId, UpdateConferenceSessionRequest request, string userId)
        {
            ...
            if (conference.IsResearchConference == true)
                throw new BadRequestException("Ch?c nang này không dành cho phiên c?a h?i ngh? nghiên c?u.");
        ...
        public async Task<ResearchConferenceBasicStepResponse> UpdateResearchConferenceBasicAsync(string conferenceId, UpdateResearchConferenceBasicRequest request)
        {
            ...
            await EnsureConferenceIsEditable(conference);
            if (conference.IsResearchConference != true)
                throw new Exception("Ph?i là conference research m?i update b?ng endpoint này du?c");
            ...
            {
                throw new BadRequestException("TicketSalestart ph?i di?n ra sau waitlist c?a h?i ngh?");
            }
        ...
        public async Task<ResearchConferencePhaseResponse> UpdateResearchConferencePhaseAsync(string phaseId, UpdateResearchConferencePhaseRequest request, string userId)
        {
            ...
            if (phaseToUpdate == null) throw new NotFoundException($"Không tìm th?y giai do?n (phase) v?i ID {phaseId}");

            var conference = await _unitOfWork.ConferenceRepository.GetConferenceByIdAsync(phaseToUpdate.ConferenceId);
            if (conference == null) throw new BadRequestException("Không tìm th?y h?i ngh? ch?a phase này.");
        ...
        public async Task<List<ResearchSessionWithMediaResponse>> AddResearchSessionsAsync(string conferenceId, AddResearchSessionsRequest request, string userId)
        {
            ...
            if (conference.CreatedBy != userId)
                throw new ForbiddenException("B?n không có quy?n thêm phiên (session) cho h?i ngh? này.");
            await EnsureConferenceIsEditable(conference);
            ...
            foreach (var group in sessionsGroupedByRoomAndDate)
            {
                ...
                for (int i = 0; i < sortedSessionsInGroup.Count - 1; i++)
                {
                    ...
                    if (currentSession.EndTime.Value > nextSession.StartTime.Value)
                    {
                        throw new BadRequestException($"D? li?u request không h?p l?: Phiên '{currentSession.Title}' (k?t thúc lúc {currentSession.EndTime:HH:mm}) b? ch?ng chéo th?i gian v?i phiên '{nextSession.Title}' (b?t d?u lúc {nextSession.StartTime:HH:mm}) trong cùng phòng và cùng ngày.");
                    }
                }
            }
            ...
            try
            {
                ...
                {
                    #region Xác th?c cho t?ng Session
                    ...
                    if (await _unitOfWork.RoomRepository.GetRoomByIdAsync(sessionRequest.RoomId) == null)
                        throw new NotFoundException($"Phòng v?i ID {sessionRequest.RoomId} không t?n t?i.");

                    // Ngày c?a session ph?i n?m trong kho?ng ngày c?a h?i ngh?
                    if (sessionRequest.Date.Value < conference.StartDate || sessionRequest.Date.Value > conference.EndDate)
                    {
                        throw new BadRequestException($"Ngày c?a phiên '{sessionRequest.Title}' ({sessionRequest.Date.Value:dd/MM/yyyy}) n?m ngoài kho?ng th?i gian di?n ra h?i ngh? ({conference.StartDate:dd/MM/yyyy} - {conference.EndDate:dd/MM/yyyy}).");
                    }
        ...
        //    // 2. Ð?m b?o dây là session c?a h?i ngh? nghiên c?u
        //    if (conference.IsResearchConference != true)
        //        throw new BadRequestException("Ch?c nang này ch? dành cho phiên c?a h?i ngh? nghiên c?u.");

        //    // 3. Xác d?nh các giá tr? cu?i cùng và xác th?c
        //    var finalDate = request.Date ?? session.SessionDate.Value;
        //    var finalStartTime = request.StartTime ?? TimeOnly.FromDateTime(session.StartTime.Value);
        //    var finalEndTime = request.EndTime ?? TimeOnly.FromDateTime(session.EndTime.Value);
        //    var finalRoomId = request.RoomId ?? session.RoomId;
        ...
        //    if (finalDate < conference.StartDate || finalDate > conference.EndDate)
        //    {
        //        throw new BadRequestException($"Ngày c?a phiên ({finalDate:dd/MM/yyyy}) ph?i n?m trong kho?ng th?i gian di?n ra h?i ngh? ({conference.StartDate:dd/MM/yyyy} - {conference.EndDate:dd/MM/yyyy}).");
        //    }

        //    var finalStartDateTime = finalDate.ToDateTime(finalStartTime);
        //    var finalEndDateTime = finalDate.ToDateTime(finalEndTime);

        //    // 4. Ki?m tra trùng l?p th?i gian
        //    await ValidateSessionTimeAvailability(finalStartDateTime, finalEndDateTime, finalRoomId, sessionId);
        //    #endregion
        ...
        //        throw new BadRequestException("Ðây là thêm price phase cho waitlist b?t bu?c ch? có th? thêm cho conferenceprice v?i iswaitlist = true");

        //    if (conferencePrice.AvailableSlot == 0)
        //        throw new Exception("B?n không th? thêm phase m? bán cho lo?i vé này vì dã h?t availableSlot r?i");
        ...
        //        throw new Exception("B?n ch? có thêm phase dành cho waitlsit khi conference status dang ? tr?ng thái ready");

        //    if (request.PricePhases == null || !request.PricePhases.Any())
        //        throw new BadRequestException("Yêu c?u ph?i ch?a ít nh?t m?t giai do?n.");
        ...
        //        throw new BadRequestException($"B?n không th? gán researchPhaseId waitlist cho pricephase hi?n t?i, c?n ph?i trong registration window c?a waitlist d? làm du?c di?u này. waitList registration window {IsWaitList.RegistrationStartDate:dd/MM/yyyy} - {IsWaitList.RegistrationEndDate:dd/MM/yyyy} hôm nay là {today:dd/MM/yyyy}");
        ...
        //            throw new BadRequestException($"Conference price v?i ID {conferencePriceId} là vé cho tác gi? c?n nam trong kho?ng registration window {IsWaitList.RegistrationStartDate:dd/MM/yyyy} - {IsWaitList.RegistrationEndDate:dd/MM/yyyy}. Vé v?i start date {phaseRequest.StartDate} và end date {phaseRequest.EndDate} không h?p l?");
        //    }

        //    var sumTotalRequestPhase = request.PricePhases.Sum(pp => pp.TotalSlot);
        //    if (sumTotalRequestPhase > conferencePrice.AvailableSlot)
        //        throw new BadRequestException("T?ng s? vé bán trong waitlist ch? có th? b?ng s? vé còn du l?i ? phase chính");
        ...
        public async Task<List<PricePhaseResponse>> AddPricePhaseForWaitList(string conferencePriceId, AddPricePhasesRequest request, string userId)
        {
            ...
            if (conference.CreatedBy != userId)
                throw new ForbiddenException("B?n không có quy?n thêm giai do?n cho lo?i vé này.");
            if (conferencePrice.IsAuthor != true)
                throw new BadRequestException("Ch?c nang này ch? dành d? thêm giai do?n cho lo?i vé 'isAuthor'.");
            if (conferencePrice.AvailableSlot == 0)
                throw new BadRequestException("Không th? thêm giai do?n m?i vì lo?i vé này dã h?t vé còn l?i.");
            ...
            foreach (var phaseRequest in request.PricePhases)
            {
                if (phaseRequest.StartDate < waitlistPhase.RegistrationStartDate || phaseRequest.EndDate > waitlistPhase.RegistrationEndDate)
                    throw new BadRequestException($"Giai do?n '{phaseRequest.PhaseName}' ph?i n?m trong kho?ng dang ký c?a waitlist ({waitlistPhase.RegistrationStartDate:dd/MM/yyyy} - {waitlistPhase.RegistrationEndDate:dd/MM/yyyy}).");
            }
            #endregion
